...
simplifyGraph' ignoreTouchable groups priority g = do
    ...
    gCanon      <- applyRule priority Canon (applyCanonRule groups) $ markEdgesUnresolved groups g
    ...
    gReact      <- applyRule priority TopLevelReact (applyTopLevelReactRule groups priority ) $ markEdgesUnresolved groups gSimplify
    let gFinal = gReact
    final <- isRuleApplied
    rulesApplied <- getAppliedRules
    if final then 
...
applyCanonRule :: (HasTypeGraph m axiom touchable types constraint ci) => Groups -> EdgeId -> TGGraph touchable types constraint ci -> m(TGGraph touchable types constraint ci)
applyCanonRule groups edgeIndex graph = do
    ...
    if not (isErrorResult result) then
            if isJust (isApplied result) then do
                let (touchables, subsitution, constraints) = fromJust (isApplied result)
                ...
                traceShowM' (Canon, result, [getConstraintFromEdge edge])
                applyCanonResult edge touchables subsitution constraints graph''
            else
                return graph'
        else
            return $ markEdgeResolved (getGroupFromEdge edge) (Resolved Canon [edgeId edge]) edge $ makeIncorrect (getGroupFromEdge edge) (Resolved Canon [edgeId edge]) (getErrorLabel result) edge graph'
            

applyCanonResult :: (HasTypeGraph m axiom touchable types constraint ci) => TGEdge constraint -> [touchable] -> [(touchable, types)] -> [constraint] -> TGGraph touchable types constraint ci -> m(TGGraph touchable types constraint ci)
applyCanonResult edge touchables subsitution constraints graph = do 
...
applyInteractRule groups curPrior edgeIndex graph = do
    ...
    foldM (\g ei -> do 
            ...
            if      isUnresolvedConstraintEdge groups e 
                ...
                -- &&  not (isInvolvedInError edge g || isInvolvedInError e g)
            ...
            then if length (getGroupFromEdge e) == length (getGroupFromEdge edge) && getGroupFromEdge e /= getGroupFromEdge edge then error $ show (e, edge) else do
                ...
                traceShowM' (Interact, result, [getConstraintFromEdge edge, getConstraintFromEdge e])
...
applyInteractResult edge (e, Error label) graph = let
    !g1 = makeIncorrect (getGroupFromEdge edge) (Resolved Interact [edgeId edge, edgeId e]) label edge graph
    ...
    in return g3
applyInteractResult edge (e, NotApplicable) graph = return graph
    ...
    if isEdgeGiven edge || not (isUnresolvedConstraintEdge groups edge) || getPriorityFromEdge edge /= curPrior || not (sameGroups (getGroupFromEdge edge) groups) then -- && priority (edgeCategory edge) == curPrior then
        ...
        foldM (\g ei -> do
            ...
            if isUnresolvedConstraintEdge groups edge && edgeId e /= edgeId edge && getGroupFromEdge e `isSuffixOf` getGroupFromEdge edge then do
                ...
                traceShowM' (Simplify, result, [getConstraintFromEdge e, getConstraintFromEdge edge])
...
applySimplifyResult eGiven (e, Error label) graph = let
    g1 = makeIncorrect (getGroupFromEdge eGiven) (Resolved Simplify [edgeId eGiven, edgeId e]) label eGiven graph
    g2 = markEdgeResolved (getGroupFromEdge eGiven) (Resolved Simplify [edgeId eGiven, edgeId e]) eGiven g1
    in return g2
applySimplifyResult eGiven (e, NotApplicable) graph = return graph
...
applyTopLevelReactRule :: (HasTypeGraph m axiom touchable types constraint ci) => Groups -> Priority -> EdgeId -> TGGraph touchable types constraint ci -> m(TGGraph touchable types constraint ci) 
applyTopLevelReactRule groups curPrior edgeIndex graph = do
    let edge = getEdgeFromId graph edgeIndex
    ...
    if isUnresolvedConstraintEdge groups edge then do 
        ...
        traceShowM' (TopLevelReact, result, [getConstraintFromEdge edge])
...
applyReactResult edge (Error label) graph = return (makeIncorrect (getGroupFromEdge edge) (Resolved TopLevelReact [edgeId edge]) label edge graph)
applyReactResult edge NotApplicable graph = return graph
...
resolvePriorityConstraints ignoreTouchable group p g 
    | odd p, p > 1 = do
        let (unresolvedUnify, unresolvedOther) = partition (isEquality . getConstraintFromEdge) $ filter (\e -> priority (edgeCategory e) == p && getGroupFromEdge e == group && isUnresolvedConstraintEdge group e) $ getUnresolvedConstraintEdgesByPriority p g
        ...
        let gErrors = foldr 
                (\e g -> makeIncorrect (getGroupFromEdge e) (Resolved Substitution []) labelResidual e g) 
                gResolved 
...
traceShowM' :: (Show a, Show constraint, Monad f) => (Rule, RuleResult a, [constraint]) -> f ()    
traceShowM' (_, NotApplicable, _) = return ()
traceShowM' x = when False (traceShowM x)